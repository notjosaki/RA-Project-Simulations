...
def main():
    ...
    texture_mars = load_texture("textura_marte.png")
    ...
    if selected_planet is None:  # Si no se selecciona nada, salir
        return
    ...
    min_zoom = -30
    max_zoom = 25
    move_speed = 5.0  # Velocidad de movimiento del planeta
    ...
    while True:
        ...
        # Capturar teclas presionadas para mover el planeta
        keys = pygame.key.get_pressed()
        if keys[K_UP]:
            selected_planet.position[1] += move_speed * clock.get_time() / 1000.0
        if keys[K_DOWN]:
            selected_planet.position[1] -= move_speed * clock.get_time() / 1000.0
        if keys[K_LEFT]:
            selected_planet.position[0] -= move_speed * clock.get_time() / 1000.0
        if keys[K_RIGHT]:
            selected_planet.position[0] += move_speed * clock.get_time() / 1000.0

        # Calcular deformación de la malla
        deformed_grid = deform_grid(grid, [selected_planet])
        ...
        # Dibujar el planeta seleccionado
        if selected_planet:
            selected_planet.draw()
